...
/* Baud rate calculated with 0.469% error. */.semr_baudrate_bits_b.abcse = 0,
		...
		.cks = 1, .brr = 161, .mddr = (uint8_t) 256,
...
adc_instance_ctrl_t g_adc_temp_ctrl;
const adc_extended_cfg_t g_adc_temp_cfg_extend = { .add_average_count =
		ADC_ADD_OFF, .clearing = ADC_CLEAR_AFTER_READ_ON, .trigger =
		ADC_START_SOURCE_DISABLED, .trigger_group_b = ADC_START_SOURCE_DISABLED,
		.double_trigger_mode = ADC_DOUBLE_TRIGGER_DISABLED, .adc_vref_control =
...
const adc_cfg_t g_adc_temp_cfg = { .unit = 1, .mode = ADC_MODE_SINGLE_SCAN,
    ...
    .p_context           = NULL,
		...
		.p_context = (void*) &NULL,
#endif
		.p_extend = &g_adc_temp_cfg_extend,
...
#if ((0) | (0))
const adc_window_cfg_t g_adc_temp_window_cfg =
{
    .compare_mask        =  0,
    .compare_mode_mask   =  0,
...
#endif
const adc_channel_cfg_t g_adc_temp_channel_cfg = { .scan_mask =
		ADC_MASK_TEMPERATURE | 0, .scan_mask_group_b = 0, .priority_group_a =
		ADC_GROUP_A_PRIORITY_OFF, .add_mask = 0, .sample_hold_mask = 0,
		.sample_hold_states = 24,
#if ((0) | (0))
    .p_window_cfg        = (adc_window_cfg_t *) &g_adc_temp_window_cfg,
#else
		.p_window_cfg = NULL,
#endif
		};
/* Instance structure to use this module. */
const adc_instance_t g_adc_temp = { .p_ctrl = &g_adc_temp_ctrl, .p_cfg =
		&g_adc_temp_cfg, .p_channel_cfg = &g_adc_temp_channel_cfg, .p_api =
		&g_adc_on_adc };
...
#endif
const adc_channel_cfg_t g_adc0_channel_cfg = { .scan_mask = ADC_MASK_CHANNEL_7
...
const uart_cfg_t g_uart6_cfg = { .channel = 6, .data_bits = UART_DATA_BITS_8,
		.parity = UART_PARITY_OFF, .stop_bits = UART_STOP_BITS_1, .p_callback =
				uart6_callback, .p_context = NULL, .p_extend =
				&g_uart6_cfg_extend,
#define RA_NOT_DEFINED (1)
#if (RA_NOT_DEFINED == RA_NOT_DEFINED)
		.p_transfer_tx = NULL,
